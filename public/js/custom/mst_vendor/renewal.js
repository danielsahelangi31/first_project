...
$(document).ready(()=> {
    ...
    $('#batal').click(function() {
        ...
        if(result.isConfirmed == true) {
            window.location.assign(`/master-vendor?request=outstanding&status=DRAFT`);
        }
        });
    });
    ...
    $('#kirim').click(function() {
          Swal.fire({
              ...
              // icon: 'warning',
              imageUrl: "/media/peralatan/refresh.svg",
              showCancelButton: true,
              cancelButtonText: "Tidak",
              confirmButtonText: 'Iya', 
              confirmButtonColor: '#045299',
...
function postData(status) {
    let id = $('#data_id').val();
    $.ajax({
        url: `/master-vendor/renewal-add/${id}`,
        type: 'post',
        beforeSend: function() {
        blockUI.block();
        },
        ...
        success: function(data) {
          ...
          Swal.fire({
            ...
            //   window.location.assign(`/master-vendor`);
              window.location.assign(`/master-vendor?request=outstanding&status=${status}`);
            });
        },
        error: function(data) {
          blockUI.release();
