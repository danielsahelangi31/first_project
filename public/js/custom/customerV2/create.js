...
$(document).ready(function() {  
  ...
  // default value joining since
  $('#joining_since').val(new Date().toDateInputValue());
  ...
  //hide npwp mandatory bullet
    ...
    //show mandatory bullet shipping & cargo  
    // $(document).on('change', 'select[id="usaha_pelanggan"]', () => {
    //   let types = $(this).val()
    ...
    //   let included = double.filter(type => types.includes(type))
    //   console.log(included, 'inculeded');
    ...
    //hide cor & DGT from county mandatory bullet
    ...
    $('.exp_document').change(function () {
      ...
      console.log(today, 'ini dari today');
      console.log(date, 'ini dari date');
...
function progressUpload(element,name,percentage){
  ...
  function uploadFile (fieldname){
    ...
    function postData(status) {
      ...
      $.ajax({
        ...
        data: {
          ...
          'tp_nm_perusahaan' : nameType() ? nameType() : [],
          'no_pmku' : $('#pmku_number').val() ? $('#pmku_number').val() : '',
          'no_sktd' : $('#no_sktd').val() ? $('#no_sktd').val() : '',
          'usaha_pelanggan' : typeUsahaPelanggan(),
          'mitra' : typeUsahaMitra().length > 0 ? typeUsahaMitra() : [],
          'shipping_line' : typeShippingLine().length > 0 ? typeShippingLine() : [],
          'cargo_owner' : cargoOwner().length > 0 ? cargoOwner() : [],
          'service_type' : serviceType().length > 0 ? serviceType() : [],
          'branch_id' : $('#first_registration_branch').val() ? $('#first_registration_branch').val() : '',
          'customer_type_id' : $('#customer_type').val() ? $('#customer_type').val() : '',
          //other table
          'npwp' : npwp(),
          ...
          'payment_type' : paymentType().length > 0 ? paymentType() : [],
          //exp document
    ...
    function billing() {
      ...
      address.forEach((val, index) => {
        billingData.push({
          ...
          'npwp_address' : val,
          'kd_npwp': generateSequenceNumber(index+1)
          // 'payment' : payment
        })
      })
      return billingData
    ...
    function documentFile() {
      return {
        'spmp' : $("input[name='spmp_file']").val() ? $("input[name='spmp_file']").val() : '',
